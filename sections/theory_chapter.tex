...
\subsection{sec:Lagrangians, Fields, and Gauge Transformations}
...
A parity inversion is equivalent to reversing a particle's momentum without reversing its spin. For a fermion field $\psi(t,x)$, a parity transform P takes $\psi(t,x)$ to $\psi(t,-x)$, and looks like $P \psi(t,x) P = e^{i \theta} \gamma^{0} \psi(t,-x)$ for some constant matrix $theta$. Given this, we find that the expression $P \bar{\psi}\psi P = \bar{\psi}\psi$ , so $\bar{\psi}\psi$ is a $\emph{scalar}$ under parity, while the expression $iP \bar{\psi}\gamma^{5}\psi P = -i\bar{\psi}\gamma^{5}\psi$ acquires a minus sign, so $i\bar{\psi}\gamma^{5}\psi$ is a $\emph{pseudoscalar}$ under parity.
...
To rectify this, we can introduce a new complex SU(2) doublet scalar field $\phi = \begin{pmatrix} \phi^{+} \\ \phi^{0} = \begin{pmatrix} \phi_{1} + i\phi_{2}  \\ \phi_{3} + i\phi_{4}$$ called the $\emph{Higgs field}$. It follows the Lagrangian
...
We note that the potential is symmetric, so we can pick a direction to "break the symmetry" in. Let us define the vacuum as $\phi_0 = \frac{1}{\sqrt{2}} \begin{pmatrix} 0 \\ v$. Then, for any arbitrary configuration of the field, we can expand the potential about the minimum as $\phi(x) = \frac{1}{\sqrt{2}} \begin{pmatrix} 0 \\ (v + h(x))$.

We can expand the kinetic term in the Lagrangian as

\begin{equation}
(D^{\mu}\phi)^{dagger}(D_{\mu}\phi) = frac{1}{2}(\partial_{mu}h)(\partial^{mu}h)+ \phi^{dagger}(\frac{1}{2}iYg_{1}B_{\mu} -\frac{1}{2}iI g_2 \Sigma \tau_{i}W_{i}_\mu)^{\dagger}(\frac{1}{2}iYg_{1}B^{\mu} -\frac{1}{2}iI g_2 \Sigma \tau_{i}W_{i}^mu) \phi
\end{equation} 

Which, using the definitions of the three tau matrices, and putting Y = I = 1, becomes

\begin{equation}
frac{1}{2}(\partial_{mu}h)(\partial^{mu}h) + frac{1}{8} \abs{\begin{bmatrix} g_{1}B_{mu}+g_{2}W_{mu}^{3} & g_{2}(W_{mu}^{1}-iW_{mu}^{2}) \\ g_{2}(W_{mu}^{1}+iW_{mu}^{2}) & g_{1}B_{mu}+g_{2}W_{mu}^{3} \end{bmatrix}\begin{bmatrix} 0 \\ v \end{bmatrix}}^{2}
...
then we get: 
\begin{equation}
frac{1}{2}(\partial_{mu}h)(\partial^{mu}h) + (frac{1}{2}vg_{2})^{2}W_mu^{+}W_mu^{-}+(frac{1}{2}v(\sqrt{{g_1}^2+{g_2}^2}))^{2}Z_{mu}
\end{equation}
...
For fermions, the coupling to the Higgs field looks like $g_{f}(\bar{f}_{L}\phi f_{R} + \phi^{dagger}\bar{f}_{R} f_{L})$ (for some constant $g_{f}$).

We note that, if we expand $\phi(x) = \frac{1}{\sqrt{2}} \begin{pmatrix} 0 \\ (v + h(x))$ as before, we find that the coupling term now looks like 
...
\begin{equation}
V = {\mu}^2\phi^{dagger}\phi + \lambda(\phi^{dagger}\phi)^{2} = frac{1}{2}{\mu}^2(v+h)^{2} + frac{1}{4}\lambda(v+h)^{4}
...
\begin{figure}[htp]
  \centering
  \label{figure}\caption{Feynman diagrams depicting the leading Higgs production modes.}
  \subfloat[Subcaption 1]{\label{figure:1}\includegraphics[width=60mm]{figures/theory_chapter/VBF.png}}
  \subfloat[Subcaption 2]{\label{figure:2}\includegraphics[width=60mm]{figures/theory_chapter/ggH.png}}
  \\
  \subfloat[Subcaption 3]{\label{figure:3}\includegraphics[width=60mm]{figures/theory_chapter/VH.png}}
  \subfloat[Subcaption 4]{\label{figure:4}\includegraphics[width=60mm]{figures/theory_chapter/ttH.png}}
  \\
\end{figure}

\begin{figure}[htp]
  \centering
  \label{figure}\caption{Feynman diagrams depicting single-top associated Higgs production.}
  \subfloat[Subcaption 1]{\label{figure:1}\includegraphics[width=60mm]{figures/theory_chapter/tWH1.png}}
  \subfloat[Subcaption 2]{\label{figure:2}\includegraphics[width=60mm]{figures/theory_chapter/tWH2.png}}
  \\
  \subfloat[Subcaption 3]{\label{figure:3}\includegraphics[width=60mm]{figures/theory_chapter/tqH1.png}}
  \subfloat[Subcaption 3]{\label{figure:3}\includegraphics[width=60mm]{figures/theory_chapter/tqH2.png}}
  \\
\end{figure}
...
\begin{figure}[htp]
  \centering
  \label{figure}\caption{Feynman diagrams depicting the leading-order processes contributing to the Higgs diphoton decay.}
  \subfloat[Subcaption 1]{\label{figure:1}\includegraphics[width=60mm]{figures/theory_chapter/toploop.png}}
  \subfloat[Subcaption 2]{\label{figure:2}\includegraphics[width=60mm]{figures/theory_chapter/Wloop1.png}}
  \subfloat[Subcaption 3]{\label{figure:1}\includegraphics[width=60mm]{figures/theory_chapter/Wloop2.png}}
\end{figure}
