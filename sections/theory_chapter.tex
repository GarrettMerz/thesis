...
Finally, Dirac fields describe all Standard Model fermions (with the possible exception of neutrinos). They are vectors as opposed to scalars, and behave according to the Lagrangian:
...
FIXME
where $\gamma^{mu}$ denotes the set of Dirac gamma matrices, and $\bar{\psi}$ denotes the transpose of the complex conjugate of the vector field,  times
FIXME


A Dirac vector field can be written in a variety of ways, but one of the most useful is that of a doublet of two spinors, one left-handed and one right-handed, that is, $\psi = \begin{pmatrix} \psi_L \\ \psi_R \end{pmatrix}$. 
