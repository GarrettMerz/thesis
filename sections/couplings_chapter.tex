...
\\\\ \gamgam\ $\vec{p}_\mathrm{T}$ projected to its thrust axis ($p_\mathrm{Tt}^{\gamma\gamma}$);
%\\\\ $p_\mathrm{Tt}^{\gamma\gamma}=|\vec{p}_\mathrm{T}^{Z\gamma} \times (\vec{p}_\mathrm{T}^{Z}-\vec{p}_\mathrm{T}^{\gamma})/|\vec{p}_\mathrm{T}^{Z}-\vec{p}_\mathrm{T}^{\gamma}||$
\\\\ Half difference between di-photon $\eta$ and sum $\eta$ of leading 2 jets ($\eta^{Zepp}$);
\\\\ $\phi^{*}_{\gamma\gamma} = \tan(\frac{\pi-|\Delta\phi_{\gamma\gamma}|}{2}) \sqrt{1-\tanh^{2}(\frac{\Delta\eta_{\gamma\gamma}}{2})}$
%\\\\ $\eta^{Zepp}=(\eta_{\gamma\gamma} - (\eta_{j1} + \eta_{j2}))/2$
...
%\\\\ Flag of the difference between ${E^{miss}_\mathrm{T}}$ computed from \\\\ di-photon vertex and hardest vertex bigger than 30 GeV;
...
\subfloat[\qqtoHqq\ ($\ge 2$-jets, $\mjj > 700\,\GeV$, $\ptHjj > 25\,\GeV$)]{\includegraphics[width=0.49\linewidth]{design/multiclass_QQ2HQQ_GE2J_MJJ_GT700_PTH_0_200_PTHJJ_0_25}\label{fig:design:BDT_VBF:multiclass}}\\
\subfloat[\qqtoHln\ ($75 \le \ptH < 150\,\GeV$)]{\includegraphics[width=0.49\linewidth]{design/multiclass_QQ2HLNU_PTV_75_150} \label{fig:design:BDT_VH:multiclass}}
\subfloat[\ttH\ ($60 \le \ptH < 120\,\GeV$)  ]{\includegraphics[width=0.49\linewidth]{design/multiclass_TTH_PTH_60_120}\label{fig:design:BDT_ttH:multiclass}}
\caption{Multiclass discriminant output distributions for four representative STXS classes. In each plot, the multiclass discriminant distribution is shown separately for events corresponding to the target STXS region (solid) and events in other STXS regions (long-dashed). The target region is further broken down into the subset of events passing the multiclass selection at detector level (orange-solid), and the subset of events that fail this selection (green-dashed).}
...
\subfloat[\qqtoHqq\ ($\ge 2$-jets, $\mjj > 700\,\GeV$, $\ptHjj > 25\,\GeV$)]{\includegraphics[width=0.49\linewidth]{design/BDT2score_QQ2HQQ_GE2J_MJJ_GT700_PTH_0_200_PTHJJ_0_25}\label{fig:design:BDT_VBF:binary}}	\\
\subfloat[\qqtoHln\ ($75 \le \ptH < 150\,\GeV$)]{\includegraphics[width=0.49\linewidth]{design/BDT2score_QQ2HLNU_PTV_75_150}\label{fig:design:BDT_VH:binary}}
\subfloat[\ttH\ ($60 \le \ptH < 120\,\GeV$)]{\includegraphics[width=0.49\linewidth]{design/BDT2score_TTH_PTH_60_120}\label{fig:design:BDT_ttH:binary}}
\caption{Binary BDT discriminant distributions in four representative STXS classes. For each class, the binary BDT discriminant distribution is shown for the target STXS region (solid), other STXS regions (dashed), and background (dots) represented by the events in the diphoton mass sidebands (105 $<~\mgg~<~$ 120 GeV or 130 $<~\mgg~<~$ 160 GeV). The vertical lines indicate the categories used in the analysis for each class. }
...
\begin{table}[]
    ...
    \begin{tabular}{l|c|c|c|c}
                 % Category  &     $S_{90}$  &  $B_{90}$  &  $f_{90}$  &  $Z_{90}$  \\
                                    Category  &      $S$  &  $B$  &  $f$  &  $Z$ \\
\toprule
...
\tHW                            &      0.80  &   2.4  &  0.25  &   0.5  \\
\tHqb                           &      0.88  &   2.7  &  0.24  &   0.5  \\
\end{tabular}}}
    \caption{For each event category, values of the expected Higgs signal ($S$) and background ($B$) yields within the smallest mass window containing 90\% of signal events, and corresponding estimates of the signal purity $f = S/(S + B)$ and the expected significance $Z = \sqrt{2( (S+B) \log(1 + S/B) - S)}$.}
...
\caption{Contribution of STXS regions in the expected event yields to each reconstruction category. The top row corresponds to the value of $S_{90}/(S_{90} + B_{90})$ in each category, where $S_{90}$ and $B_{90}$ are respectively the total number of signal (including all STXS regions) and background events expected in the smallest \mgg\ range containing 90\% of the signal yield. Other entries correspond to the percentage fraction for the contribution of a given STXS truth bin to the Higgs signal yield in each analysis category. Entries for the STXS regions targeted by each analysis category are outlined in black if this value is above 15\%. }
...
\caption{Distribution of the diphoton invariant mass \mgg\ in four representative STXS categories both in Monte Carlo background templates (histograms) and in data (black points). The data are shown excluding the region $120 < \mgg < 130\,\GeV$ containing the signal. In categories~\ref{fig:design:bkg_ggH} and~\ref{fig:design:bkg_VBF}, the $\gamma \gamma$, $\gamma j$ (green) and $jj$ (magenta) background contributions are shown stacked and colored differently. }
...
\end{tabular}
  ...
  \caption{The impact of groups of systematic uncertainties to the total error on the observed cross section times branching ratio. This is shown as the uncertainty due to each group of systematic uncertainties ($\Delta \sigma$), as a fraction of the total observed cross section ($\sigma$). For each group of uncertainties, asymmetric errors are used.}
...
\begin{figure}
    ...
    \caption{The inclusive diphoton invariant mass distribution of events from all analysis categories. The events in each category are weighted by $\ln(1+S/B)$, where $S$ and $B$ are the expected signal and background yields in this category within the smallest \mgg\ window containing 90\% of the signal events. The weighted sum of the signal plus background fits is represented by the solid line, while the blue dotted line represents the weighted sum of the background functional forms. }
...
\caption{Combined diphoton invariant mass distributions for categories targeting the same production processes. The events in each category are weighted by $\ln(1+S/B)$, where $S$ and $B$ are the expected signal and background yields in this category within the smallest \mgg\ window containing 90\% of the signal events. The weighted sum of the signal plus background fits is represented by the solid line, while the blue dotted line represents the weighted sum of the background functional forms. Only Higgs boson events from the targeted production processes in each category are considered as signal events in these plots; Higgs boson events from other processes are treated as part of the background.}
...
\end{tabular}
\end{center}
\caption{Best-fit values and uncertainties for the production cross sections of the Higgs boson times the $H \rightarrow \gamma\gamma$ branching ratio. The total uncertainties are decomposed into components for data statistics (Stat.), and systematic uncertainties (Syst.). SM predictions are shown for the cross section of each production process.}
  ...
  \small
   \renewcommand{\arraystretch}{1.35}
    \begin{tabular}{l|rlll@{ }|l@{ }}
      \hline \hline
      \multirow{2}{*}{STXS region $(\sigma_i \times \Byy)$} & \multicolumn{1}{c}{Value} & \multicolumn{3}{c|}{ Uncertainty [fb]} & \multicolumn{1}{c}{SM prediction} \\
...
\begin{figure}[tp!]
  ...
  \vspace{-10pt}
  \caption{Measured cross sections times branching fraction for the cross sections in each analysis category. The black error bars, blue boxes and yellow boxes show the total, systematic, and statistical uncertainties in the measurements, respectively. Similarly, the gray bands show the theory uncertainties in the predictions.}
