...
\caption{Multiclass discriminant output distributions for four representative STXS classes. In each plot, the multiclass discriminant distribution is shown separately for events corresponding to the target STXS region (solid) and events in other STXS regions (long-dashed). The target region is further broken down into the subset of events passing the multiclass selection at detector level (orange), and the subset of events that fail this selection (green).}
